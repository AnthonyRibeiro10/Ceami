const louvores = [
  ...
  { nome: "Nosso General é Cristo", tom: "C", ministro: "Keila" },
  { nome: "Primeira Essência", tom: "G", ministro: "Keila" },
  { nome: "Como prometeste", tom: "G", ministro: "Keila" },
  { nome: "Como na primeira vez", tom: "G", ministro: "Keila" },
  { nome: "Adore", tom: "G", ministro: "Keila" },
  { nome: "Vou te adorar", tom: "D", ministro: "Keila" },
  { nome: "Essência da Adoração", tom: "D", ministro: "Keila" }    
];
...
function buscarLouvores() {
    ...
    const louvoresFiltrados = louvores.filter(louvor => {
        return (tomSelecionado === 'Qualquer' || louvor.tom === tomSelecionado) &&
            (ministroSelecionado === 'Qualquer' || louvor.ministro === ministroSelecionado) &&
            (louvor.nome.toLowerCase().includes(palavraChave));
    });
    ...
    // Mostra os louvores filtrados
    mostrarLouvores(louvoresFiltrados);
}

function mostrarLouvores(listaLouvores) {
    const tabelaLouvores = document.getElementById("tabelaLouvores");
    tabelaLouvores.innerHTML = ""; // Limpa a tabela antes de mostrar os louvores
    ...
    if (listaLouvores.length === 0) {
        ...
        listaLouvores.forEach((louvor, index) => {
            ...
            colunaNome.style.cursor = "pointer"; // Define o estilo do cursor como ponteiro para indicar que é clicável
            colunaNome.addEventListener("click", function() {
                ...
                window.open(url, "_blank"); // Abre o URL do louvor em uma nova aba
            });
            linhaLouvor.appendChild(colunaNome);

            const colunaTom = document.createElement("td");
            colunaTom.textContent = louvor.tom;
            colunaTom.style.cursor = "pointer"; // Define o estilo do cursor como ponteiro para indicar que é clicável
            colunaTom.addEventListener("click", function() {
                const louvorNome = louvor.nome.replace(/\s/g, "+"); // Substitui espaços por "+" no nome do louvor
                const url = `https://www.cifraclub.com.br/?q=${louvorNome}&t=${louvor.tom}`;
                window.open(url, "_blank"); // Abre o URL do tom do louvor em uma nova aba
            });
